...
char PATH[MAXARGS] = "/usr/bin/"; //execve로 함수를 가져올 위치
...
int test_history(char** argv, int argc, char* cmdline); //!!나 !#이 있는지 확인하고 대체하는 함수
...
void eval(char* cmdline) {
    ...
    int argc = 0;        //command에서 나눈 command 단위의 개수
    
    int indx[MAXARGS] = { 0, };
    int count = 0;
    char* pipe_cmd[MAXARGS][MAXARGS];
    int status;

    strcpy(buf, cmdline); //copy command line into buf
    bg = parseline(buf, argv, &argc, indx); //2. PARSING(파이프 라인 고려 포함)
    ...
    if (test_history(argv, argc, cmdline)) { //argv[0]에 !!이나 !#이 있는지 검사 --> 있으면 해당 라인으로 대체
        argc = 0;
        strcpy(buf, cmdline); //copy command line into buf
        bg = parseline(buf, argv, &argc, indx); //2-1. PARSING AGAIN(파이프 라인 고려 포함)
        if (argv[0] == NULL) return; //ignore empty lines
    };
    ...
    if (--count) {
        for (i = 0; i <= count; i++) {
            k = 0;
            while (argv[j] != NULL) pipe_cmd[i][k++] = argv[j++];
            j++;
        }

        if ((pid = Fork()) == 0)
            pipe_command(pipe_cmd, 0, STDIN_FILENO);
...
int parseline(char* buf, char** argv, int* argc, int* index) {
    ...
    int pipe_num = 0;        //number of pipe
    int i;
    char* quote;

    buf[strlen(buf) - 1] = ' ';  //command끝에 있는 개행문자('\n')를 공백으로 대체
    while (*buf && (*buf == ' ')) //처음 글자가 나오기 전의 공백들은 무시한다
        buf++;

    int blank_cnt = 0;
    for (i = 1; i < strlen(buf); i++) {
        if (buf[i] == '|') {
            index[pipe_num++] = blank_cnt + 1;
            buf[i] = ' ';
        }
        else if (buf[i - 1] == ' ' && buf[i] != ' ') ++blank_cnt;
    }
    
    for (i = 0; i < pipe_num; i++) index[i] += i;
    ...
    while (delim = strchr(buf, ' ')) {
        ...
        *delim = '\0';
        ...
        while (*buf && (*buf == ' ')) /* Ignore spaces */
            buf++;
        if ((*argc) == index[i]) { argv[(*argc)++] = NULL; i++; }
    }
    argv[(*argc)] = NULL;
...
void pipe_command(char* argv[][MAXARGS], int cnt, int fin) {
    int pipefd[2]; //파일스크립터를 저장하는 변수
    pid_t pid;

    if (argv[cnt + 1][0]) { //마지막 명령어가 아닌 경우
        if (pipe(pipefd) < 0) { //파이프를 만들어 fd에 스크립터를 저장
            printf("pipe error!\n");
            exit(1);
        }
        if ((pid = fork()) == 0) { //자식 프로세스를 만들고
            close(pipefd[0]);
            if (fin != STDIN_FILENO) { 
                dup2(fin, STDIN_FILENO); //이전 프로세스의 입력용 파일 디스크립터를 표준 입력으로 연결
                close(fin);
            }
            dup2(pipefd[1], STDOUT_FILENO); //쓰기용 파일 디스크립터를 표준 출력으로 연결
            close(pipefd[1]);

            if (!builtin_command(argv[cnt])) { //실행
                if (execve(strcat(PATH, argv[cnt][0]), argv[cnt], environ) < 0) {
                    printf("%s: %s\n", argv[cnt][0], strerror(errno)); //해당 부분이 오류 메시지 출력하는 부분
                    exit(0);
                }
            }
            exit(0);
        ...
        close(pipefd[1]);
        pipe_command(argv, cnt + 1, pipefd[0]); //다음 명령어 실행
        int status;
        if (Waitpid(pid, &status, 0) < 0)
            unix_error("waitfg: waitpid error\n");
    }
    else { //마지막 명령어인 경우
        dup2(fin, STDIN_FILENO); //이전꺼의 파일 스크립터를 표준 입력으로 받아서
        close(fin);
        if (!builtin_command(argv[cnt])) { //마지막 명령어 실행
            if (execve(strcat(PATH, argv[cnt][0]), argv[cnt], environ) < 0) {
                printf("%s: %s\n", argv[cnt][0], strerror(errno)); //해당 부분이 오류 메시지 출력하는 부분
                exit(0);
            }
